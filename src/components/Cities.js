    ...
    submitData = async (e) => {
        ...
        try {
            ...
            this.setState({
                ...
                let imageApi = `https://key=${process.env.REACT_APP_BACKEND_API}`
                let map = await axios.get(imageApi)
                this.setState({
                    image: map.config.url
                })
            })
            let weatherApi = `http:localhost:8000/weather?lat=${this.state.latitude}&lon=${this.state.longitude}`
            let weatherGet = await axios.get(weatherApi).then(res => {
                this.setState({
                    weather: res.data,
                    show: true
                })
            ...
            let moviesApi = `http:localhost:8000/weather?lat=${this.state.latitude}&lon=${this.state.longitude}`
            let moviesrGet = await axios.get(moviesApi).then(res => {
                this.setState({
                    movies: res,
